document.addEventListener('DOMContentLoaded', function() {
  ...
  function parseCredentialString(inputString) {
    ...
    let message = 'The required policy indicates that the credential must be of type ';
    message += credentialTypes.join(', ') + ', in addition to including the following values:\n\n';
  
    // Añadir los campos y sus valores al mensaje
    fields.forEach((field, index) => {
      message += `In the field "${field.slice(field.lastIndexOf('.') + 1)}": ${values[index]}\n`;
    });
  ...
  function fetchCredentials() {
    fetch('https://umu-webWallet:8445/vpTokenDetails')
  ...
  function displayCredentials(credentials) {
    ...
    if (Object.keys(credentials).length === 0) {
      showModal("There are no credentials that comply with the policy");
    }
  ...
  function showModal(message) {
    ...
    emitButton.textContent = 'Start issuance process';
  ...
  function selectCredential(credential) {
    ...
    .then(response => response.text()) 
    .then(text => {
      window.location.href = `https://umu-verifier:8444/verify?TokenJWT=${text}`;
    })
    .catch(error => {
