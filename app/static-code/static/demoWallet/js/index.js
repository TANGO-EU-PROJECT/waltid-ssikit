document.addEventListener('DOMContentLoaded', function() {
  ...
  function fetchCredentials() {
    fetch('https://umu-webWallet:<WALLET_PORT>/vpTokenDetails')
    ...
    fetch('https://umu-webWallet:<WALLET_PORT>/validCredentials')
  ...
  function selectCredential(credential) {
    ...
    fetch('https://umu-webWallet:<WALLET_PORT>/selectCredential', {
        method: 'POST',
        headers: {
    ...
    .then(response => response.text()) 
    .then(text => {
      window.location.href = `https://umu-verifier:<VERIFIER_PORT>/verify?TokenJWT=${text}`;
    })
    .catch(error => {
