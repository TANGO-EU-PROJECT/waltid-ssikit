...
object OIDC4VPService {
    ...
    fun postSIOPResponse(
        ...
        val result = HTTPRequest(HTTPRequest.Method.POST, req.redirectionURI).apply {
            ...
            log.debug("Sending SIOP response to {}\n {}", it.uri, it.query)
        }.send().also {
            log.debug { "SIOP RESPONSE SENT: received ${it.statusCode}" }
        }
    ...
    fun postSIOPResponse_UMU(
        ...
        val result = HTTPRequest(HTTPRequest.Method.POST, redirect_uri).apply {
            if (mode == CompatibilityMode.EBSI_WCT) {
